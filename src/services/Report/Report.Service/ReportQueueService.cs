...
namespace Report.Service
{
    public class ReportQueueService : IHostedService, IDisposable
    {
        ...
        public ReportQueueService(IServiceProvider provider)
        {

            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;

            var options = provider.GetService<IOptions<ServiceOptions>>();

            _apiClient = provider.GetService<IApiClientService>();
            _service = provider.GetService<IReportService>();
            ...
            _rabbitMQBus.SendReceive.ReceiveAsync<Models.Report>(Constant.Q_REPORT, async (x) =>
            {
                if (x.ID != null)
                {
                    ...
                    using (var package = new ExcelPackage())
                    {
                        ...
                        _service.UpdateReport(x.ID.Value, $"/upload/{x.ID}.xlsx");
                    }
                }
            });
